...
function Uninstall-OperaSilently {
    ...
    if ($InstallInfo.Method -eq "winget") {
        ...
        try {
            $result = & winget uninstall --id $InstallInfo.WingetId --silent --accept-source-agreements --force 2>&1
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host "   $S_CROSS [$BrowserLabel] Winget uninstall failed (exit code $LASTEXITCODE)."
                Write-Host "   $S_WARN  Attempting registry fallback..."
                # Try registry fallback
                $regInfo = Get-OperaInstallMethod -WingetId "N/A" -BrowserLabel $BrowserLabel
                if ($regInfo.Method -eq "registry") {
                    return Uninstall-OperaSilently -InstallInfo $regInfo -BrowserLabel $BrowserLabel
                }
                return $false
            }
        }
        catch {
            Write-Host "   $S_CROSS [$BrowserLabel] Winget uninstall error: $($_.Exception.Message)"
            return $false
        }
    }
    elseif ($InstallInfo.Method -eq "registry") {
        Write-Host "   $S_TRASH  [$BrowserLabel] Uninstalling via registry UninstallString..."
        try {
            $uninstCmd = $InstallInfo.UninstallString

            # Opera's UninstallString is typically:
            #   "C:\Users\user\AppData\Local\Programs\Opera\launcher.exe" /uninstall
            # We need to add /silent to suppress UI
            if ($uninstCmd -notmatch "/silent") {
                $uninstCmd = "$uninstCmd /silent"
            }

            # Parse the executable and arguments
            ...
            elseif ($uninstCmd -match '^(\S+)\s*(.*)$') {
                ...
                Write-Host "   $S_CROSS [$BrowserLabel] Cannot parse UninstallString: $uninstCmd"
                return $false
            }
            ...
            if ($process.ExitCode -eq 0) {
                Write-Host "   $S_CHECK [$BrowserLabel] Uninstalled successfully via installer."
                Start-Sleep -Seconds 3
                return $true
            }
            else {
                Write-Host "   $S_CROSS [$BrowserLabel] Uninstaller exited with code $($process.ExitCode)."
                return $false
            }
        }
        catch {
            Write-Host "   $S_CROSS [$BrowserLabel] Registry uninstall error: $($_.Exception.Message)"
            return $false
        }
    }
...
function Invoke-MalExtScan {
    ...
    if ($threats.Count -gt 0) {
        ...
        Remove-MaliciousExtensions -ThreatIds $threatIds

        # ---- GPO POLICY ENFORCEMENT - Block in Chrome & Edge ----------------
        Block-MaliciousExtensionsGPO -ThreatIds $threatIds
        ...
        if ($operaThreats.Count -gt 0) {
            ...
            Invoke-OperaRemediation -AffectedBrowsers $affectedOperaBrowsers
        }
    }
    else {
