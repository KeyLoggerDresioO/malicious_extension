...
function Get-OperaInstallMethod {
    <#
    .SYNOPSIS
        Detect how a specific Opera variant is installed: winget or registry.
    .PARAMETER WingetId
        The winget package ID (e.g. "Opera.Opera" or "Opera.OperaGX").
    .PARAMETER BrowserLabel
        Display label ("Opera" or "Opera GX").
    .OUTPUTS
        Hashtable with Method ("winget", "registry", "none") and details.
    #>
    param(
    ...
    try {
        $wingetCheck = $null
        # Redirect stderr to suppress winget noise
        $wingetCheck = & winget list --id $WingetId --accept-source-agreements 2>$null
        if ($LASTEXITCODE -eq 0 -and $wingetCheck -match $WingetId) {
            Write-Host "   $S_CHECK [$BrowserLabel] Detected installation via winget ($WingetId)"
            return @{ Method = "winget"; WingetId = $WingetId }
        }
    }
    catch {
        # winget not available or failed - fall through to registry
    }

    # ---- Fallback: Registry UninstallString -------------------------------------
    $registryPaths = @(
    ...
    $searchName = if ($BrowserLabel -eq "Opera GX") { "Opera GX" } else { "Opera" }
    ...
    foreach ($regPath in $registryPaths) {
        ...
        foreach ($key in $subKeys) {
            try {
                ...
                # Match "Opera Stable" or "Opera GX Stable"
                $isMatch = $false
                if ($BrowserLabel -eq "Opera GX" -and $displayName -like "*Opera GX*") {
                    $isMatch = $true
                }
                elseif ($BrowserLabel -eq "Opera" -and $displayName -like "*Opera Stable*") {
                    $isMatch = $true
                }
                elseif ($BrowserLabel -eq "Opera" -and $displayName -eq "Opera" -and $displayName -notlike "*GX*") {
                    $isMatch = $true
                }

                if ($isMatch -and $props.UninstallString) {
                    Write-Host "   $S_CHECK [$BrowserLabel] Detected installation via registry: $displayName"
                    return @{
                        Method          = "registry"
    ...
    Write-Host "   $S_WARN  [$BrowserLabel] Could not detect installation method."
    return @{ Method = "none" }
}
...
function Uninstall-OperaSilently {
    ...
    .DESCRIPTION
        Three-tier uninstall strategy:
        1. winget with --scope user
        2. Registry UninstallString with Chromium --force-uninstall flag
        3. Nuclear: direct directory removal + registry cleanup
    .PARAMETER InstallInfo
        Hashtable from Get-OperaInstallMethod.
    .PARAMETER BrowserLabel
        Display label ("Opera" or "Opera GX").
    .OUTPUTS
    ...
    #>
    param(
        [Parameter(Mandatory = $true)] [hashtable]$InstallInfo,
        [Parameter(Mandatory = $true)] [string]$BrowserLabel
    )

    $uninstalled = $false

    # ---- Tier 1: Winget (if detected via winget) --------------------------------
    if ($InstallInfo.Method -eq "winget") {
        Write-Host "   $S_TRASH  [$BrowserLabel] Uninstalling via winget..."
        try {
            # Opera is typically per-user, so we need --scope user
            $result = & winget uninstall --id $InstallInfo.WingetId --silent --scope user --accept-source-agreements --force 2>&1
            if ($LASTEXITCODE -eq 0) {
                Write-Host "   $S_CHECK [$BrowserLabel] Uninstalled successfully via winget."
                Start-Sleep -Seconds 3
                return $true
            }
            else {
                Write-Host "   $S_CROSS [$BrowserLabel] Winget uninstall failed (exit code $LASTEXITCODE). Trying native installer..."
            }
        }
        catch {
            Write-Host "   $S_CROSS [$BrowserLabel] Winget error: $($_.Exception.Message). Trying native installer..."
        }

        # Fallback to registry method
        $regInfo = Get-OperaInstallMethod -WingetId "N/A" -BrowserLabel $BrowserLabel
        if ($regInfo.Method -eq "registry") {
            $InstallInfo = $regInfo
        }
        else {
            # Skip to nuclear option
            return Remove-OperaDirectly -BrowserLabel $BrowserLabel
        }
    }

    # ---- Tier 2: Native installer with Chromium --force-uninstall flag -----------
    if ($InstallInfo.Method -eq "registry") {
        Write-Host "   $S_TRASH  [$BrowserLabel] Uninstalling via native installer (--force-uninstall)..."
        try {
            $uninstCmd = $InstallInfo.UninstallString

            # Parse the executable path from UninstallString
            $exe  = $null
            $baseArgs = $null
            ...
            if (-not $exe -or -not (Test-Path $exe)) {
                Write-Host "   $S_CROSS [$BrowserLabel] Cannot find uninstaller exe. Using direct removal..."
                return Remove-OperaDirectly -BrowserLabel $BrowserLabel
            }

            # Opera uses a Chromium-based installer. The correct silent flags are:
            #   --uninstall       - trigger uninstall mode (may already be in baseArgs)
            #   --force-uninstall - skip UI dialog, force silent uninstall
            #   --system-level    - for system-wide installs (optional)
            # WRONG: /silent, /S, /quiet - these are NSIS flags and do NOT work

            # Build the argument list - ensure both flags are present
            $argParts = @()
            if ($baseArgs) { $argParts += $baseArgs }
            if ($baseArgs -notmatch "--force-uninstall") {
                $argParts += "--force-uninstall"
            }
            $finalArgs = $argParts -join " "

            Write-Host "   $S_SEARCH Running: `"$exe`" $finalArgs"

            $process = Start-Process -FilePath $exe -ArgumentList $finalArgs `
                -WindowStyle Hidden -Wait -PassThru -ErrorAction Stop

            if ($process.ExitCode -eq 0) {
                Write-Host "   $S_CHECK [$BrowserLabel] Uninstalled successfully via native installer."
                Start-Sleep -Seconds 3
                return $true
            }
            else {
                Write-Host "   $S_WARN  [$BrowserLabel] Native installer exited with code $($process.ExitCode). Using direct removal..."
                return Remove-OperaDirectly -BrowserLabel $BrowserLabel
            }
        }
        catch {
            Write-Host "   $S_CROSS [$BrowserLabel] Native installer error: $($_.Exception.Message). Using direct removal..."
            return Remove-OperaDirectly -BrowserLabel $BrowserLabel
        }
    }
...
function Remove-OperaDirectly {
    ...
    .DESCRIPTION
        When the native uninstaller fails or shows a GUI despite --force-uninstall,
        this function forcefully removes the Opera program files and cleans up
        the Windows Uninstall registry entries.
        Profile data in AppData\Roaming is NOT touched (preserving bookmarks etc.).
    .PARAMETER BrowserLabel
        Display label ("Opera" or "Opera GX").
    .OUTPUTS
...
function Install-OperaSilently {
    ...
    .PARAMETER OriginalMethod
        Original install method ("winget" or "registry") to try matching reinstall path.
    .OUTPUTS
    ...
    param(
        [Parameter(Mandatory = $true)] [string]$WingetId,
        [Parameter(Mandatory = $true)] [string]$BrowserLabel,
        [Parameter(Mandatory = $true)] [string]$OriginalMethod
    )
...
function Invoke-OperaRemediation {
    ...
    .DESCRIPTION
        ...
        2. Detects the installation method (winget vs registry)
        3. Silently uninstalls the affected browser(s)
        4. Silently reinstalls the affected browser(s)
        User profile data (bookmarks, passwords, history) is preserved because the
        AppData\Roaming\Opera Software profile folder is NOT removed.
    ...
    foreach ($browser in $uniqueBrowsers) {
        ...
        $installInfo = Get-OperaInstallMethod -WingetId $wingetId -BrowserLabel $browser

        if ($installInfo.Method -eq "none") {
            Write-Host "   $S_WARN  [$browser] Not installed or cannot detect - skipping reinstall."
            Write-Host ""
            continue
        }

        # Uninstall
        $uninstalled = Uninstall-OperaSilently -InstallInfo $installInfo -BrowserLabel $browser
        if ($uninstalled) {
            $totalUninstalled++
            ...
            $reinstalled = Install-OperaSilently `
                ...
                -OriginalMethod $installInfo.Method
            if ($reinstalled) {
                $totalReinstalled++
            }
        }
    ...
    Write-Host ""
    Write-Host "$S_WARN  NOTE: User profile data (bookmarks, passwords) was preserved."
    Write-Host ""
